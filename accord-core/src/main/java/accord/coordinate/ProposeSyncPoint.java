...
public class ProposeSyncPoint extends Propose<SyncPoint>
{
    ...
    void onAccepted()
    {
        if (txnId.rw() == ExclusiveSyncPoint)
        {
            ...
            // Looks to be fine
            node.send(acceptTracker.nodes(), id -> new Apply(id, acceptTracker.topologies(), acceptTracker.topologies(), txnId.epoch(), txnId, route, txn, txnId, deps, txn.execute(txnId, txnId, null), txn.result(txnId, txnId, null)));
            callback.accept(new SyncPoint(txnId, deps, route, true), null);
        }
        else
        {
            ...
            BlockOnDeps.blockOnDeps(node, txnId, txn, route, deps, (result, throwable) -> {
                ...
                if (throwable != null)
                    ...
                    callback.accept(new SyncPoint(txnId, deps, route, false), null);
            });
